...
public class DepartamentoDAO implements IDepartamentoDAO {
    ...
    public boolean inserir(Departamento depat) throws SQLException {
        ...
        try {
            ...
            smt.execute();
            inseriu = true;
            JOptionPane.showConfirmDialog(null, "Departamento inserido com sucesso!", "Status da inserção", -1);
        } catch (Exception e) {
            String str = e.toString();
            if (str.contains("NOME_UNIQUE")) {
                JOptionPane.showConfirmDialog(null, "Não é possivel adicionar um departamento com o nome '" + nome +"' pois um departamento contém um mesmo nome", "Erro ao adicionar um departamento", -1);
            } else if (str.contains("SIGLA_UNIQUE")) {
                JOptionPane.showConfirmDialog(null, "Não é possivel adicionar um departamento com a sigla '" + sigla +"' pois um departamento contém a mesma sigla", "Erro ao adicionar um departamento", -1);
            } else {
                JOptionPane.showConfirmDialog(null, e.getMessage(), "Erro ao adicionar um departamento", -1);
            }
        }
